...
The original workflow forward-filled missing monthly futures prices and then calculated percentage returns. That procedure generated 14 rows in which Brent, gold and copper returns were all exactly zero, despite non-zero XLE and XME returns. These rows were excluded because they mechanically attenuate commodity-equity relationships and are not credible observations of three futures contracts all being unchanged in the same month.
...
The same model is re-estimated using lagged headline CPI thresholds of 2.68%, 3.0%, 3.5% and 4.0%. This checks whether the result depends on a single regime boundary.

## Limitations

- The sample contains 89 usable monthly observations.
